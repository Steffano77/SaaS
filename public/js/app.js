...
async function carregarProdutos() {
  ...
  tbody.innerHTML = todosProds.map(p => {
    ...
    return `<tr data-prod-id="${p.id}">
      ...
      <td class="right hide-mobile">${parseFloat(p.custo_unitario).toLocaleString('pt-BR',{style:'currency',currency:'BRL'})}</td>
      <td class="right hide-mobile" style="font-weight:600">${parseFloat(p.preco_venda||0).toLocaleString('pt-BR',{style:'currency',currency:'BRL'})}</td>
      <td class="center hide-mobile" style="${p.validade ? 'color:var(--orange-600);font-weight:600' : 'color:var(--slate-400)'}">${validade}</td>
