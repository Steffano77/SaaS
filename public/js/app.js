...
async function carregarProdutos() {
  ...
  tbody.innerHTML = todosProds.map(p => {
    ...
    return `<tr data-prod-id="${p.id}">
      <td class="td-nome"><div class="td-main">${p.nome}</div><div class="td-sub">${p.codigo_barras || '—'}</div></td>
      <td class="col-hide-mobile" style="color:var(--slate-600)">${p.categoria || '—'}</td>
      <td class="right td-mono td-saldo">${fmtQtd(p.estoque_atual)} ${p.unidade}</td>
      ...
      <td class="center td-status">${status}</td>
      <td class="right td-acoes" style="white-space:nowrap;">
