...
router.get('/clientes-faturado/documento/:documento/saldo',   auth, authPremium, wrap(clienteFaturadoCtrl.saldo));
...
router.get('/comandas/historico',      auth, authPremium, exigirFuncionario(['gerente']), wrap(comandaCtrl.listar));
...
router.get('/caixa/:id/reimprimir-fechamento', auth, authPremium, wrap(caixaCtrl.reimprimirFechamento));
...
router.post('/atendentes/:id/resetar-pin', auth, authPremium, wrap(atendenteCtrl.resetarPin));
router.get('/atendentes/:id/exportar',   auth, authPremium, wrap(atendenteCtrl.exportarExcel));
...
router.get('/admin/padarias', auth, authAdmin, wrap(async (req, res) => {
  ...
  const [rows] = await db.query(`
    SELECT id, nome, email, plano, plano_expira_em, plano_bloqueado, role, ativo, criado_em,
      DATEDIFF(plano_expira_em, CURDATE()) AS dias_para_expirar,
      (SELECT COUNT(*) FROM produtos WHERE padaria_id = padarias.id) AS total_produtos
    FROM padarias ORDER BY plano_expira_em IS NULL, plano_expira_em ASC, criado_em DESC`);
  res.json(rows);
}));
